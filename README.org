...
*** jsonpipe
Simple implementation of jsonpipe (https://github.com/zacharyvoase/jsonpipe) in PHP.

*** commit.sh
Commit script for SVN.
Automatically adds/removes new/missing files (use svn:ignore for files you don't want to include).
Commits also changes to the repositories designed as externals.
